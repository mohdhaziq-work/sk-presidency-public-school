...
import { collection, addDoc, getDocs, getDoc, query, where, orderBy, deleteDoc, doc, updateDoc, Timestamp, setDoc, limit } from 'firebase/firestore';
...
export async function getNotices(cls:string,sec:string):Promise<NoteSubmission[]>{ try{const qq=query(collection(db,'notes'),where('class','==',cls.toUpperCase()),where('section','==',sec.toUpperCase()),where('type','==','Notice'),orderBy('createdAt','desc'),limit(50));const s=await getDocs(qq);return s.docs.map(d=>({id:d.id,...d.data()}as NoteSubmission))}catch(e){return[]}}
export async function getMyNotes(sid:string):Promise<NoteSubmission[]>{ try{const qq=query(collection(db,'notes'),where('studentId','==',sid),orderBy('createdAt','desc'),limit(200));const s=await getDocs(qq);return s.docs.map(d=>({id:d.id,...d.data()}as NoteSubmission))}catch(e){return[]}}
...
export async function getStudentsByClass(cls:string,sec?:string):Promise<StudentProfile[]>{ try{let qq=query(collection(db,'students'),where('class','==',cls.toUpperCase()),limit(200));if(sec)qq=query(qq,where('section','==',sec.toUpperCase()));const s=await getDocs(qq);return s.docs.map(d=>({id:d.id,...d.data()}as StudentProfile))}catch(e){return[]}}
export async function updateStudent(id:string,updates:Partial<StudentProfile>){ try{await updateDoc(doc(db,'students',id),updates as any);return true}catch(e){return false}}
...
export async function getAllTeachers():Promise<TeacherProfile[]>{ try{const s=await getDocs(collection(db,'teachers'));return s.docs.map(d=>({id:d.id,...d.data()}as TeacherProfile))}catch(e){return[]}}
export async function addTeacher(d:Omit<TeacherProfile,'id'>){ const r=await addDoc(collection(db,'teachers'),d);return r.id;}
export async function deleteTeacher(id:string){ try{await deleteDoc(doc(db,'teachers',id));return true}catch(e){return false}}

// CURRICULUM
export async function getCurriculum(cls:string):Promise<SubjectCurriculum[]>{ try{const qq=query(collection(db,'curriculum'),where('class','==',cls.toUpperCase()),limit(100));const s=await getDocs(qq);return s.docs.map(d=>({id:d.id,...d.data()}as SubjectCurriculum))}catch(e){return[]}}
export async function addCurriculum(d:Omit<SubjectCurriculum,'id'>){ const r=await addDoc(collection(db,'curriculum'),d);return r.id;}
