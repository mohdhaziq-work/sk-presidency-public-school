...
function Login({ onSuccess }: { onSuccess: (p: StudentProfile) => void }) {
  ...
  return (
    ...
    <div className="w-full max-w-[400px] bg-white border border-[#E2E8F0] rounded-[10px] p-8 shadow-[0_8px_32px_rgba(0,0,0,.06)]" style={{animation:'scaleIn .4s cubic-bezier(0.16,1,0.3,1)'}}>
      ...
      <h2 className="text-xl font-bold text-center mb-1 text-[#0F172A]">Student Portal</h2>
      <p className="text-xs text-[#64748B] text-center mb-5">Sign in to your dashboard</p>
      <div className="flex border-b border-[#E2E8F0] mb-5">
        <button onClick={()=>{setTab('id');clr();if(step===2)back()}} className={`flex-1 pb-2.5 text-[13px] font-semibold border-b-2 transition ${tab==='id'?'text-[#2563EB] border-[#2563EB]':'text-[#64748B] border-transparent'}`}>Student ID</button>
        <button onClick={()=>{setTab('mob');clr()}} className={`flex-1 pb-2.5 text-[13px] font-semibold border-b-2 transition ${tab==='mob'?'text-[#2563EB] border-[#2563EB]':'text-[#64748B] border-transparent'}`}>Mobile</button>
      </div>
      {err&&<div className="bg-[#FEF2F2] text-[#DC2626] p-2.5 rounded-[10px] text-xs mb-3 font-medium">{err}</div>}
      {ok&&<div className="bg-[#ECFDF5] text-[#059669] p-2.5 rounded-[10px] text-xs mb-3 font-medium">{ok}</div>}
      {tab==='id'&&<form onSubmit={idLogin}>
        <div className="mb-3.5"><input type="text" value={sid} onChange={e=>setSid(e.target.value)} className="w-full p-[11px_14px] border border-[#E2E8F0] rounded-[10px] text-sm bg-[#F9FAFB] outline-none focus:border-[#2563EB] focus:bg-white" placeholder="Student ID or Roll Number" autoComplete="off"/></div>
        <div className="mb-5"><input type="password" value={pw} onChange={e=>setPw(e.target.value)} className="w-full p-[11px_14px] border border-[#E2E8F0] rounded-[10px] text-sm bg-[#F9FAFB] outline-none focus:border-[#2563EB] focus:bg-white" placeholder="Password (DOB: DDMMYYYY)" autoComplete="off"/></div>
      ...
      {tab==='mob'&&step===1&&<div>
        <div className="mb-5"><input type="tel" value={mobile} onChange={e=>setMobile(e.target.value.replace(/\D/g,''))} maxLength={10} className="w-full p-[11px_14px] border border-[#E2E8F0] rounded-[10px] text-sm bg-[#F9FAFB] outline-none focus:border-[#2563EB] focus:bg-white" placeholder="Registered Mobile Number" autoComplete="off"/></div>
        <button onClick={findMobile} disabled={loading} className="w-full py-3 bg-[#2563EB] text-white rounded-[10px] text-sm font-semibold hover:bg-[#1D4ED8] transition disabled:opacity-50">{loading?'':'Find My Account'}</button>
      </div>}
      {tab==='mob'&&step===2&&<div>
        <button onClick={back} className="flex items-center gap-1 text-[#2563EB] text-[11px] font-semibold bg-none border-none cursor-pointer mb-2.5 p-0">← Change mobile number</button>
        <div className="flex flex-col gap-1.5 mb-3 max-h-[200px] overflow-y-auto">
          {matches.map((s,i)=>(
            <div key={i} onClick={()=>{setSelIdx(i);clr()}} className={`flex items-center gap-2.5 p-2.5 border rounded-[10px] cursor-pointer transition ${selIdx===i?'border-[#2563EB] bg-[rgba(37,99,235,.04)]':'border-[#E2E8F0] bg-white hover:bg-[#f8faff] hover:border-[#2563EB]'}`}>
              <div className="w-9 h-9 rounded-full bg-[#2563EB] text-white flex items-center justify-center text-[15px] font-semibold flex-shrink-0">{(s.full_name||'S').charAt(0).toUpperCase()}</div>
              <div className="flex-1 min-w-0"><div className="text-[13px] font-semibold truncate">{xe(s.full_name)}</div><div className="text-[10px] text-[#64748B]">Class {xe(s.class)}-{xe(s.section||'A')} | {xe(s.student_id||s.id)}</div></div>
              <div className={`w-[18px] h-[18px] rounded-full border-2 flex items-center justify-center flex-shrink-0 ${selIdx===i?'bg-[#2563EB] border-[#2563EB]':'border-[#E2E8F0]'}`}>{selIdx===i&&<span className="text-white text-[10px] font-bold">✓</span>}</div>
            </div>
          ))}
        </div>
        <div className="mb-3.5"><input type="password" value={mpw} onChange={e=>setMpw(e.target.value)} className="w-full p-[11px_14px] border border-[#E2E8F0] rounded-[10px] text-sm bg-[#F9FAFB] outline-none focus:border-[#2563EB] focus:bg-white" placeholder="Password (DOB: DDMMYYYY)" autoComplete="off"/></div>
        <button onClick={mobLogin} disabled={loading} className="w-full py-3 bg-[#059669] text-white rounded-[10px] text-sm font-semibold hover:shadow-[0_4px_16px_rgba(5,150,105,.25)] transition disabled:opacity-50">{loading?'':'Sign In as Selected'}</button>
      </div>}
      <div className="text-center mt-4 text-[10px] text-[#94A3B8]"><Link href="/" className="text-[#94A3B8] hover:text-[#2563EB]">Back to School Website</Link><br/><span className="text-[9px] mt-1 block">Default password: Date of Birth (DDMMYYYY)</span></div>
    </div>
    </div>
  );
}
...
function Dash({profile,uid,onLogout}:{profile:StudentProfile;uid:string;onLogout:()=>void}){
  ...
  return <div className="min-h-screen bg-gray-50">
    <header className="sticky top-0 z-40 bg-white/85 backdrop-blur-xl border-b border-gray-200 h-14 flex items-center justify-between px-5"><Link href="/" className="flex items-center gap-2 font-bold text-sm">SKPPS<span className="text-[10px] text-gray-400 ml-1">{cls}-{sec}</span></Link><button onClick={onLogout} className="text-xs font-semibold text-red-500 hover:bg-red-50 px-3 py-1.5 rounded-lg">Logout</button></header>
    <div className="flex min-h-[calc(100vh-3.5rem)]">
      <aside className="w-52 bg-white border-r border-gray-200 hidden md:flex flex-col sticky top-14 h-[calc(100vh-3.5rem)]"><div className="px-4 py-3 text-[10px] font-bold uppercase tracking-[0.12em] text-gray-400">Student Portal</div>{TABS.map(t=>(<button key={t.id} onClick={()=>setTab(t.id)} className={`flex items-center gap-2 px-4 py-2.5 mx-2 rounded-lg text-xs font-medium text-left ${tab===t.id?'bg-blue-50 text-blue-600':'text-gray-500 hover:bg-gray-50'}`}>{t.l}</button>))}<div className="mt-auto p-4 border-t text-center text-[10px] text-gray-400">SK Presidency</div></aside>
    ...
    <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-gray-200 h-16 flex md:hidden z-40">{TABS.map(t=>(<button key={t.id} onClick={()=>setTab(t.id)} className={`flex-1 flex flex-col items-center justify-center gap-0.5 text-[9px] font-semibold ${tab===t.id?'text-blue-600':'text-gray-400'}`}>{t.l}</button>))}</nav>
    {viewN&&<Vw note={viewN} onClose={()=>setViewN(null)}/>}
  </div>;
}

function HomeTab({p,n,onNav}:{p:StudentProfile;n:NoteSubmission[];onNav:(t:Tab)=>void}){return <div><h2 className="text-lg font-bold mb-1">Welcome, {(p.full_name||'Student').split(' ')[0]}</h2><p className="text-xs text-gray-400 mb-6">Class {p.class}-{p.section||'A'} | {p.house||'Earth'} House | Roll {p.roll_no||'-'}</p><div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">{[{v:p.class,l:'Class'},{v:p.house||'Earth',l:'House'},{v:p.roll_no||'-',l:'Roll'},{v:p.parent_phone||'-',l:'Contact'}].map((s,i)=>(<div key={i} className="bg-white border border-gray-200 rounded-xl p-4 text-center hover:shadow-md transition"><div className="text-2xl font-extrabold">{s.v}</div><div className="text-[9px] text-gray-400 mt-1">{s.l}</div></div>))}</div>{n.length>0&&<div className="bg-white border rounded-xl mb-4"><div className="px-4 py-3 border-b font-semibold text-sm">Recent Notices</div><div className="p-4">{n.map((x,i)=>(<div key={i} className="py-2 border-b border-gray-50 last:border-0"><strong className="text-xs">{x.title}</strong><p className="text-[11px] text-gray-500">{x.content?.substring(0,100)}</p></div>))}</div></div>}<div className="flex gap-2"><button onClick={()=>onNav('notes')} className="px-4 py-2 bg-blue-600 text-white text-xs rounded-lg">View Notes</button><Link href="/" className="px-4 py-2 border text-xs rounded-lg">Website</Link></div></div>;}
function ProfTab({p}:{p:StudentProfile}){const r=[['Name',p.full_name],['Father',p.father_name],['Mother',p.mother_name],['Class',p.class+'-'+(p.section||'A')],['Roll',p.roll_no],['House',p.house],['Gender',p.gender],['DOB',p.date_of_birth],['Phone',p.parent_phone],['Blood',p.blood_group],['SR No',p.sr_no||p.student_id]];return <div><h2 className="text-lg font-bold mb-4">My Profile</h2><div className="bg-white border rounded-xl p-6"><div className="flex gap-5 mb-6"><div className="w-16 h-16 rounded-full bg-blue-600 text-white flex items-center justify-center text-2xl font-bold">{(p.full_name||'S').charAt(0)}</div><div><h3 className="font-bold text-lg">{p.full_name}</h3><p className="text-xs text-gray-400">{p.class}-{p.section||'A'}</p></div></div><div className="grid grid-cols-2 md:grid-cols-3 gap-3">{r.map(([l,v],i)=>(<div key={i} className="flex gap-2 text-xs"><span className="font-semibold text-gray-400 uppercase text-[10px] w-16">{l}</span><span>{v||'-'}</span></div>))}</div></div></div>;}
function NotTab({n}:{n:NoteSubmission[]}){return <div><h2 className="text-lg font-bold mb-4">Notices</h2>{n.length?<div className="space-y-3">{n.map((x,i)=>(<div key={i} className="bg-white border rounded-xl p-5"><div className="flex justify-between mb-1"><strong className="text-sm">{x.title}</strong><span className="text-[10px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded">Notice</span></div><p className="text-xs text-gray-500 whitespace-pre-wrap">{x.content}</p><div className="flex gap-3 text-[10px] text-gray-400 mt-3 pt-2 border-t"><span>{x.studentName||'Management'}</span><span>{x.createdAt?.toDate?.()?.toLocaleDateString()||''}</span></div></div>))}</div>:<p className="text-center text-gray-400 py-10">No notices.</p>}</div>;}
