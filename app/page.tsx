    ...
    <div className="min-h-screen bg-[#F8F9FA]">
      ...
      <header className="sticky top-0 z-50 bg-white/85 backdrop-blur-2xl border-b border-[#DADCE0] h-14 flex items-center justify-between px-5">
        ...
        <div className="flex items-center gap-2">
          <Link href="/student" className="text-xs font-semibold bg-[#1A1C1E] text-white px-4 py-2 rounded-full hover:bg-[#4285F4] hover:shadow-lg transition-all active:scale-[0.97]">Student Login</Link>
          ...
          <button onClick={() => setMenuOpen(!menuOpen)} className="sm:hidden relative w-9 h-9 rounded-xl bg-gradient-to-br from-[#4285F4] to-[#3367D6] flex items-center justify-center shadow-md hover:shadow-lg hover:from-blue-700 hover:to-[#174EA6] transition-all active:scale-95">
      ...
      <div className={`sm:hidden fixed inset-0 z-[60] transition-opacity duration-300 ${menuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`absolute top-0 right-0 bottom-0 w-[300px] max-w-[85vw] bg-white shadow-2xl flex flex-col ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`} style={{transition:'transform 400ms cubic-bezier(0.16,1,0.3,1)'}}>
          <div className="bg-gradient-to-r from-[#4285F4] to-[#3367D6] px-5 py-6 flex-shrink-0">
            <div className="flex items-center justify-between mb-4">
              <Link href="/" onClick={() => setMenuOpen(false)} className="flex items-center gap-2.5">
                ...
                <div className="text-white"><div className="font-bold text-sm leading-tight">SK Presidency</div><div className="text-[9px] text-[#A8C7FA]">Public School</div></div>
              </Link>
            ...
            <div className="flex gap-2">
              <Link href="/student" onClick={() => setMenuOpen(false)} className="flex-1 text-center py-2 bg-white text-[#3367D6] rounded-lg text-[11px] font-bold hover:bg-[#E8F0FE] transition">Student Login</Link>
          ...
          <div className="flex-1 overflow-y-auto py-3 px-4">
            ...
            {NAV.map((n, i) => (
              <Link key={n.href} href={n.href} onClick={() => setMenuOpen(false)} className={`flex items-center gap-3 px-3 py-3 rounded-xl mb-0.5 text-sm font-medium transition-all ${n.href==='/' ? 'bg-[#E8F0FE] text-[#4285F4] font-semibold shadow-[inset_0_0_0_1px_rgba(37,99,235,.2)]' : 'text-gray-600 hover:bg-[#F8F9FA] hover:text-gray-800'}`}>
                <span className={`w-2 h-2 rounded-full flex-shrink-0 ${n.href==='/'?'bg-[#4285F4] scale-125 shadow-[0_0_8px_rgba(37,99,235,.4)]':'bg-gray-300'}`}/>{n.label}{n.href==='/'&&<span className="ml-auto text-[10px] text-[#669DF6]">● Active</span>}
              </Link>
            ))}
            ...
            {SIDEBAR.map((s, i) => (
              <Link key={s.h} href={s.h} onClick={() => setMenuOpen(false)} className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl mb-0.5 text-xs font-medium text-gray-500 hover:bg-[#F8F9FA] hover:text-gray-700 transition-all">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={s.c} strokeWidth="2" opacity="0.4"><path d={s.i}/></svg>{s.l}
              </Link>
            ))}
          </div>
          <div className="flex-shrink-0 px-4 py-3 border-t border-[#E8EAED] bg-[#F8F9FA]"><div className="flex items-center justify-between text-[9px] text-gray-400"><span>CBSE: 2133231</span><span>Astra Infotech</span></div></div>
        </div>
      </div>

      {/* ═══ NEWS TICKER ═══ */}
      <div className="bg-[#4285F4] text-white overflow-hidden"><div className="flex items-center gap-2 px-5 py-2 max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-10 items-center">
            <div className="animate-fade-up">
              ...
              <div className="flex gap-3 flex-wrap">
                ...
                <Link href="/admission" className="bg-white border border-[#DADCE0] text-gray-700 px-5 py-3 rounded-xl font-semibold text-sm hover:shadow-lg transition-all">Admissions 2026-27</Link>
              </div>
            </div>
            <div className="hidden md:flex justify-center">
              <div className="w-44 h-44 rounded-full bg-gradient-to-br from-[#D2E3FC] to-[#D2E3FC] flex items-center justify-center animate-[float_6s_ease-in-out_infinite] shadow-xl shadow-[#D2E3FC]/30 animate-float hover:shadow-2xl hover:shadow-[#A8C7FA]/40 transition-shadow duration-500">
      ...
      <div className="max-w-6xl mx-auto px-5 py-12"><div className="flex gap-8">
        <aside className="w-52 flex-shrink-0 hidden lg:block sticky top-[72px] self-start">
          <div className="bg-gradient-to-r from-[#4285F4] to-[#3367D6] text-white text-[10px] font-bold uppercase tracking-[0.1em] px-3.5 py-2.5 rounded-t-lg shadow-sm">Quick Links</div>
          <div className="bg-white border border-t-0 border-[#DADCE0] rounded-b-lg overflow-hidden shadow-sm">
            {SIDEBAR.map(s=>(
              <Link key={s.h} href={s.h} className="flex items-center gap-2.5 px-3.5 py-2.5 text-xs font-medium border-b border-[#E8EAED] last:border-0 transition-all hover:bg-[#F8F9FA] text-gray-600 hover:text-gray-800 hover:border-l-[3px] hover:border-l-gray-200 hover:translate-x-1">
      ...
      {/* ═══ CTA ═══ */}
      <section className="bg-gradient-to-r from-blue-700 to-[#174EA6] py-16 text-center"><div className="max-w-6xl mx-auto px-5"><h2 className="text-2xl md:text-3xl font-extrabold text-white mb-2 animate-fade-up">Join the SK Presidency Family</h2><p className="text-[#A8C7FA] mb-8 max-w-md mx-auto">Admissions open for Play Group to Class XII.</p><div className="flex gap-3 justify-center flex-wrap"><Link href="/admission" className="bg-white text-[#3367D6] px-5 py-3 rounded-xl font-semibold text-sm hover:shadow-xl hover:-translate-y-0.5 transition-all active:scale-[0.97]">Apply for Admission</Link><Link href="/contact" className="border border-[#669DF6]/40 text-white px-5 py-3 rounded-xl font-semibold text-sm hover:bg-white/10 hover:-translate-y-0.5 transition-all active:scale-[0.97]">Contact Us</Link></div></div></section>

      {/* ═══ YOUTUBE VIDEO + REGISTRATION ═══ */}
      <section className="bg-white py-16">
        <div className="max-w-6xl mx-auto px-5">
          <div className="grid md:grid-cols-2 gap-8 items-start">
            <div className="rounded-2xl overflow-hidden shadow-lg border border-[#DADCE0]">
              <div className="bg-gradient-to-r from-[#4285F4] to-[#3367D6] px-5 py-3">
              ...
              <div className="p-3 bg-[#F8F9FA] border-t">
                <a href="https://www.youtube.com/channel/UCyYHfZf4gxt_aKjEqqqkmYg" target="_blank" rel="noopener" className="text-[11px] text-[#4285F4] font-semibold hover:underline">Subscribe to YouTube Channel &rarr;</a>
              </div>
            </div>
            <div className="space-y-4">
              <div className="bg-gradient-to-r from-[#4285F4] to-[#3367D6] rounded-2xl p-6 text-white">
                <h3 className="font-bold text-lg mb-2">Registration Open For Session 2026-2027</h3>
                <p className="text-[#D2E3FC] text-sm mb-4">Contact <strong className="text-white">86017 35757, 86017 38180</strong></p>
                <div className="flex gap-3">
                  <a href="https://www.skpresidency.com/Prospectus.pdf" className="bg-white text-[#3367D6] px-4 py-2.5 rounded-xl text-xs font-bold hover:bg-[#E8F0FE] transition">Prospectus</a>
      ...
      <div className="bg-white border-y border-[#DADCE0] py-3 overflow-hidden">
      ...
      <footer className="bg-gray-900 text-gray-400 py-12"><div className="max-w-6xl mx-auto px-5">
        <div className="grid md:grid-cols-4 gap-6 mb-8">
          ...
          <div><h4 className="text-white text-[10px] font-bold uppercase tracking-[0.1em] mb-2">Portals</h4><Link href="/student" className="block text-sm py-0.5 text-[#669DF6] hover:text-blue-300 font-medium">Student Login</Link><Link href="/staff" className="block text-sm py-0.5 text-[#669DF6] hover:text-blue-300 font-medium">Staff Login</Link><Link href="/governingbody" className="block text-sm py-0.5 text-gray-500 hover:text-white">Governing Body</Link><Link href="/career" className="block text-sm py-0.5 text-gray-500 hover:text-white">Careers</Link></div>
        </div>
